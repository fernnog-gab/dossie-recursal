...
## 🏛️ Contexto de Negócio
Este painel foi desenvolvido para atuar como uma ferramenta de apoio analítico nos gabinetes judiciais da Justiça do Trabalho. O ecossistema de uso funciona em duas etapas integradas:
1. **Extração e Análise (IA):** Modelos de Inteligência Artificial leem documentos jurídicos complexos da fase recursal (Sentença, Recurso Ordinário, Contrarrazões) e extraem dados processuais estruturados em um formato JSON.
2. **Visualização e Consolidação (Painel Front-end):** A aplicação web consome esse JSON e instancia um painel interativo. O usuário interage com a trilha de julgamento, verifica pressupostos de admissibilidade, reorganiza tópicos e, ao final, exporta um dossiê HTML unificado. Este arquivo final atua como um registro consolidado, salvo em nuvem, para consulta ágil durante sessões de julgamento ou na elaboração do voto.
...
* **🕒 Sistema de Versionamento:** Histórico interativo e dinâmico isolado no arquivo `versions.js`, garantindo rastreabilidade das atualizações.
...
5. Consulte o botão no cabeçalho para visualizar o histórico de versões da ferramenta.
6. Ao finalizar a análise da trilha, clique em **Salvar e Baixar Cópia** para exportar o dossiê consolidado.
...
* `versions.js` - Banco de dados isolado com o histórico de atualizações e documentação de *features* lançadas.
* `README.md` - Esta documentação central.

## 🆕 Últimas Atualizações
* **Versionamento Dinâmico:** Implementação de botão de controle de versão no cabeçalho com histórico flutuante alimentado via `versions.js`.
